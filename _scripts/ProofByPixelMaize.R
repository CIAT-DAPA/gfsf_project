...
########### PARTE B
#cargar files
crops<- c("Maize")  #, "Bean", "Maize", "Rice" "Rice", "Soybean"
...
for(p in 1:length(pots)){
      ...
      if(nrow(by_pots[[p]]) >= 1){
            ...
            #hacer graficas
            png(filename = paste(copyPix,"Maize","_","IRR_",pots[p],"_","trend",".png",sep=""), 
            ...
            cat(paste(" MAIZE IRRI FPU: ", pots[[p]], " does not have varieties\n", sep = ""))
      }
...
for(p in 1:length(pots)){
      ...
      if(nrow(by_pots[[p]]) >= 1){
            ...
            #hacer graficas
            png(filename = paste(copyPix,"Maize","_","RA_",pots[p],"_","trend",".png",sep=""), 
                width = 20, height = 12, units = 'in', res = 100)
            require(ggplot2)
            n<-ggplot(data=by_pots[[p]], aes(x=year,y=val,colour=pix))+ 
                  geom_line(aes(group=pix))+
            ...
            cat(paste(" MAIZE RA FPU: ", pots[[p]], " does not have varieties\n", sep = ""))
      }

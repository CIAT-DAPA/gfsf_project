...
plot(i,vertex.label=V(i)$vertex.names, layout=layout.fruchterman.reingold,
     ...
     main="PPA Network" )#,edge.width=(degi/5)*2 ) #, main=paste("Visualización de ",ti, sep = "" ))
...
E(i)$arrow.size <- .9
# E(i)$width <- degi
plot(i,vertex.label=V(i)$vertex.names, layout=layout.kamada.kawai,
     edge.color="grey", vertex.label.dist=0.5,vertex.label.font=2,
     vertex.label.cex=0.9, vertex.frame.color="#ffffff", 
     main="PPA Network" )#,edge.width=(degi/5)*2 ) #, main=paste("Visualización de ",ti, sep = "" ))
# legend(x=-1.2, y=1.2,c("Publico", "Privado", "ONG", "Aso.Prod", "Coop.Inter", "Otro"), pch=21,
...
E(i)$arrow.size <- .9
# E(i)$width <- degi
plot(i,vertex.label=V(i)$vertex.names, layout=layout.random,
     edge.color="grey", vertex.label.dist=0.5,vertex.label.font=2,
     vertex.label.cex=0.9, vertex.frame.color="#ffffff", 
     main="PPA Network" )#,edge.width=(degi/5)*2 ) #, main=paste("Visualización de ",ti, sep = "" ))
# legend(x=-1.2, y=1.2,c("Publico", "Privado", "ONG", "Aso.Prod", "Coop.Inter", "Otro"), pch=21,

...
db_dir <- './_data/_cassava_data/periods_median/' # Using median as summary measure
...
db_time <- lapply(1:length(db_periods), function(i){
  ...
  z$X <- NULL
  colnames(z) <- c('Reporter', 'Partner', 'Median') # Median
...
real_exports <- db_time[db_time$Median > 0,]; rownames(real_exports) <- 1:nrow(real_exports)

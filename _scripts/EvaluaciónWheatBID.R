...
copy<- c("C:/Users/CEGONZALEZ/Documents/BIDCarlos/BIDsecundVersion/Wheat_Future/")
...
for (v in 1:length(variedades)){
 for (g in 1:length(gcm)){
   ...
   #Computador personal
   write.csv(rend_fpu3,paste(copy,variedades[v],'_',gcm[g],'_FPU.csv',sep=''),row.names=T)
   } 
}  
...
grd3<- c("//dapadfs/workspace_cluster_3/bid-cc-agricultural-sector/BID_Evaluacion_Economica/08-Cells_toRun/matrices_cultivo/")
copy<- c("C:/Users/CEGONZALEZ/Documents/BIDCarlos/BIDsecundVersion/Wheat_Historical/")
...
for (v in 1:length(variedades)){
    ...
    #Computador personal
    write.csv(rend_fpu3,paste(copy,variedades[v],'_',"WFD",'_FPU.csv',sep=''),row.names=T)
